...
class AddCondoPopup(tk.Frame):
    ...
    def _submit_cb(self):
        ...
        data = {}
        data['square_feet'] = int(self.square_footage)
        data['year_built'] = int(self.year_built)
        data['number_of_rooms'] = int(self.number_of_rooms)
        data['number_of_bathrooms'] = int(self.number_of_bathrooms)
        data['city'] = str(self.city)
        data['selling_agent'] = str(self.selling_agent)
        data['yearly_property_tax'] = float(self.yearly_property_tax)
        data['monthly_strata_fee'] = int(self.monthly_strata_fee)
        data['pets_allowed'] = int(self.pets_allowed)
        data['type'] = self.home_type
        data['id'] = int(self.home_id)
